...
void main() {
  group('PagingStatusExtension', () {
    ...
    test('returns loadingFirstPage status when loading first page with no items and no error', () {
      pagingState = PagingState<int, String>();
      expect(pagingState.status, PagingStatus.loadingFirstPage);
    });

    test('returns firstPageError status when first page has no items and there is an error', () {
    ...
    test('returns ongoing status when items exist, there is no error, and more pages are available', () {
      pagingState = PagingState<int, String>(
        pages: const [
          ['Item 1']
        ],
    ...
    test('returns subsequentPageError status when items exist and there is an error', () {
      pagingState = PagingState<int, String>(
        pages: const [
          ['Item 1']
        ],
    ...
    test('returns completed status when items exist and no more pages are available', () {
      pagingState = PagingState<int, String>(
        pages: const [
          ['Item 1']
        ],
